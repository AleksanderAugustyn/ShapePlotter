...
class ShapeAnalyzer:
    ...
    @staticmethod
    def find_nearest_point(plot_x: np.ndarray, plot_y: np.ndarray, angle: float) -> Tuple[float, float]:
...
class NuclearShapePlotter:
    ...
    def update_plot(self, _=None):
        ...
        along_x_length = (calculator.calculate_radius(0.0) +
                          calculator.calculate_radius(np.pi))
        along_y_length = (calculator.calculate_radius(np.pi / 2) +
                          calculator.calculate_radius(-np.pi / 2))
